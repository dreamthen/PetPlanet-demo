...
import detailAPI from './detail_service';
...
@connect((state) => {
  ...
  return {
     ...
     */
    async getPetDetailInfoHandler(...args) {
      await dispatch(homeAPI.getPetDetailRequest.apply(this, args));
    },
    /**
     * 对此宠物交易进行收藏
     * @returns {Promise<void>}
...
class Detail extends Component {
  ...
  onShareAppMessage({from, target, webViewUrl}) {
    ...
    const {id, nickName, avatarUrl, page} = this.$router.params;
    return {
      title: `逼疯的铲屎官-${title}`,
  ...
  render() {
    ...
    const {title, isLoading, cost, content, collected, images, area, contactInfo, tags, avatarUrl, nickName, wantCount} = detailStore;
    return (
      <ScrollView
      ...
      >
        ...
        <View className='pet-detail-content'>
          <View className='pet-detail-content-price'>
            ...
            {cost}
            <Text className='pet-detail-content-like'>
              <Text className='pet-detail-content-like-count'>{wantCount}</Text>人想要
            </Text>
        ...
        <View className='pet-detail-images'>
          {
            images && images.length && images.map((imageItem, imageIndex) => {
              return <Image
                ...
                src={imageItem}
                onClick={(e) => onPreviewImage(imageItem)}
                onLoad={onImageLoad}
              />
            })
          }
